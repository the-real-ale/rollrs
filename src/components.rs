...
impl Graph1D for Hist1D {
    ...
    fn set_bin(&mut self, bin: u16, value: f32) {
        let index = if bin >= self.data.len() as u16 {
            self.data.len() - 1
        } else {
            bin as usize
        };
...
impl TextBox {
    pub fn new<T>(component: ComponentData, text: T, show_border: bool) -> Self
    where
        T: ToString,
    {
        Self {
...
impl Component for TextBox {
    fn draw(&self, mut stdout: &Stdout) -> Result<()> {
        drawterm::scroll(self.get_height() + 1)?;
        self.draw_border(stdout)?;
        self.draw_formatted_lines(
            stdout,
            &textwrap::wrap(&self.text, self.get_width() as usize - 3),
        )?;
        drawterm::reset_cursor(stdout)?;
        stdout.flush()?;
        drawterm::scroll(1)?;
