...
pub fn draw_solid_rectangle(x0: u16, y0: u16, width: u16, height: u16, stdout: &Stdout) -> Result<()>{
    for y in y0..y0 + height{
        for x in x0..x0 + width{
            draw_char_at(x, y, '█', stdout)?;
        }
...
pub fn draw_vertical_line(x0: u16, y0: u16, height: f32, stdout: &Stdout) -> Result<()> {
    ...
    if height - height.floor() >= 0.125{
        draw_char_at(x0, y0 - (height as u16), get_vertical_fraction(height - height.floor()), stdout)?;
    }
    Ok(())
}
...
fn get_vertical_fraction(normalize: f32) -> char {
    if normalize < 0.125 {
        return ' ';
    }
